...
def get_response(qid):
    response = conn.query(f"SELECT answer, count(*) as cnt FROM answers WHERE qid = {qid} GROUP BY qid,answer;")
    return response
...
option = st.selectbox('Select your question', questions, index=0, label_visibility="collapsed")
...
with st.container():
    if st.button('Add a reply',use_container_width=True):
        ...
        with c1:
            ...
            qid = ''
            t_input = st.empty()
            answer = t_input.text_input(label='input',label_visibility="collapsed",placeholder='Reply goes here...')
            ...
            if st.button('Add a reply'):
                ...
                if answer != '' and st.session_state.clicked == True:
                    with conn.session as c:
                        c.execute(text(f"INSERT INTO answers (qid, answer) VALUES ('{qid}','{answer}')"))
                        c.commit()
